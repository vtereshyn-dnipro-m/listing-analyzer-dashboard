...
| [unique_keys.txt](unique_keys.txt) | уникальные ключи и индексы — под проверку `ON CONFLICT` |
...
Вторая проверка — про запись: `ON CONFLICT (колонки)` работает только
при уникальном ключе ровно по этим колонкам, иначе Postgres отвечает
«there is no unique or exclusion constraint matching the ON CONFLICT
specification». Ключи берутся из `unique_keys.txt`, а миграции
подмешиваются ТОЛЬКО для таблиц, которых в снимке ещё нет: иначе
множество копило бы историю. Миграция 07.09 объявила
`figma_products UNIQUE (figma_file_key, figma_node_id)`, миграция 08.09
этот ключ снесла — но объявление осталось в файле, и первая версия
проверки считала снесённый ключ живым (поймано мутацией, не глазами).
