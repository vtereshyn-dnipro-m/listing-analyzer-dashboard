...
**3. Нулевой остаток — не всегда причина подавления.** Если блокирующая
проблема появилась раньше, чем остаток обнулился, причина в ней, а
`suppression_cause = out_of_stock` только маскирует её: сток кончился
как следствие простоя, а не как причина. Проверяется по `first_seen`
блокирующих строк пары относительно момента обнуления.

Практическое следствие для текущего кода: по `out_of_stock` боль
в Диагнозе намеренно не создаётся (дубль с правилом по остаткам) —
[services/issues.py:335](services/issues.py:335). Если Amazon назвал
причиной сток, а на паре есть более ранняя блокирующая строка, товар
покажет в Каталоге «Нет товара» и не даст боли вовсе. Прежде чем
доверять `suppression_cause`, стоит сверить его с `first_seen`
остальных строк пары.
