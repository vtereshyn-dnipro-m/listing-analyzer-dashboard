...
Промах `t()` теперь не только показывает сырой ключ, но и копится
в `i18n.missing_keys()`; сайдбар из этого делает вывод «интерфейс новее
модулей — нужен ребут». Отличать забытый перевод от незавершённого
деплоя на глаз невозможно, а лечится второе за полминуты.
...
ищутся по СТРОКАМ файла. Вторая половина — про деплой: промахи `t()`
копятся, и сайдбар говорит «интерфейс новее модулей, нужен ребут»
вместо сырых ключей на экране.
