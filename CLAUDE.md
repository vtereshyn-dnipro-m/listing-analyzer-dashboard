...
тоже. Сейчас паритет полный: по 691 ключу в каждом языке, дублей нет.
...
текущей выборке, а не по базе): «Требуют замены» (правило
`title_over_limit`, по умолчанию), «Все» (весь каталог) и
«Отправленные» (есть удачная запись в `listing_push_log`). Фильтр
...
состояние и длина результата. Карточка раскрывается под строкой и
осталась прежней. Массовые действия — одной панелью над списком, все
работают по ОТМЕЧЕННЫМ строкам текущей выборки: «Выбрать все»,
