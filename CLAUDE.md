...
`threshold.min_ctr` в `app_settings`, по умолчанию 0.3%.
Таблицы: `sqp_reports`.
...
3. **Тема из `streamlit/config.toml` не применяется**: Streamlit читает
   `.streamlit/config.toml` (с точкой). Палитра в приложении держится на
   константах страниц и `inject_fonts()`, а не на этом файле.

4. **Папка `streamlit/` в корне перекрывает пакет** при обычном
   `python -c "import streamlit"` из корня репозитория: каталог становится
   namespace-пакетом раньше установленного. На `streamlit run app.py` это не
   влияет (пакет уже импортирован CLI), но ломает скрипты и тесты — запускай
   их из другой директории.

5. **Строки мимо `t()`** (правило 5) остались в: `pages/guide.py`
   (последний caption с именем и дедлайном), `pages/catalog.py` (подписи
   линейки «резать» / «свободно»), `pages/photo.py` и `pages/matrix_setup.py`
   (тексты ошибок в `st.error`), а также тексты болей, которые
   `collect_rows()` пишет в `diagnosis` по-русски — на витрине они
   переводятся по `rule_id`, но в базе лежат на одном языке.
