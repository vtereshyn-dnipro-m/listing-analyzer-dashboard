...
# Слова «для всех принятых» из верхних кнопок убраны: под фильтром они
# были неправдой — кнопка берёт принятые ПОД ФИЛЬТРОМ. Признаком области
# осталось число: наверху оно есть и равно размеру выборки, в карточке
# его нет вовсе, а фильтр называет строка состояния рядом.
labels = " | ".join(str(b.label) for b in top_dl + by_key("push-open"))
check("у верхних кнопок в подписи счётчик выборки", "· 3" in labels)
