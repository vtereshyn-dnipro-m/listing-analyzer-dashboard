# -*- coding: utf-8 -*-
"""
main.py — скрипт Listing Suite: навигация, сайдбар, страницы.
Три языка: EN / RU / UA (i18n.py), Диагноз по умолчанию, иконки Material.

Запускается НЕ напрямую, а через app.py: там Streamlit поднимается
как ASGI-приложение (`st.App("main.py", routes=…)`), чтобы рядом
со страницами жил HTTP-маршрут для плагина Figma. Сам скрипт от этого
не меняется — `streamlit run app.py`, как и раньше.
"""
